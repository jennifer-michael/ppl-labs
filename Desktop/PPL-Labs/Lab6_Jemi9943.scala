...
object Lab6 extends jsy.util.JsyApplication {
   ...
   *
   * Partner: Loic Guegan
   * Collaborators: Cameron Taylor, Jace Conflenti, Alex Doran, Jessica Petty
   */
  ...
  object REParser extends Parsers {
    ...
    def atom(next: Input): ParseResult[RegExpr] = {
      if (next.atEnd) Failure("Uh oh", next)
      else( next.first, next.rest) match{
        ...
        case ('(', next) => re(next) match{
          case Success(rest, next) => (next.first, next.rest) match{
            case (')', next) => Success(rest, next)
            case _ => Failure("Not end paren", next)
          }
          case fail => fail
        }
        case (c, next) if !delimiters.contains(c) => Success(RSingle(c), next)
        case _ => Failure("Missing atom", next)
      }
    }
  ...
  def retest(re: RegExpr, s: String): Boolean = {
    def test(re: RegExpr, chars: List[Char], sc: List[Char] => Boolean): Boolean = (re, chars) match {
      ...
      case (RConcat(re1, re2), _) => test(re1, chars, { suffix => test(re2, suffix, sc)})
      case (RUnion(re1, re2), _) => test(re1, chars, sc) || test(re2, chars, sc)
      case (RStar(re1), _) => sc(chars) || test(re1, chars, {suffix => if (suffix.size >= chars.size) false else test(RStar(re1), suffix, sc)})

      /* Extended Operators */
      case (RAnyChar, Nil) => false
      case (RAnyChar, _ :: t) => sc(t)
      case (RPlus(re1), _) => test(RConcat(re1, RStar(re1)), chars, sc)
      case (ROption(re1), _) => if (sc(chars)) true else test(re1, chars, sc)
